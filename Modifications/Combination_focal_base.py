...
from models.models_to_finetune import deit_small_patch16_224,deit_base_patch16_224
...
class MyModel(torch.nn.Module):
    ...
    def __init__(self):
        ...
        transformer = deit_base_patch16_224(pretrained=True, use_top_n_heads=6,use_patch_outputs=False)
        transformer_features = transformer.head.in_features

        self.backbone2 = transformer#torch.nn.Sequential(*(list(transformer.children())[:-1]))
...
plt.plot(losses)
plt.savefig('combnation_base.png')
# ## Testing
...
torch.save(model.state_dict(),'combination_base.ckpt')